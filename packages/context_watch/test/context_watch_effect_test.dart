...
void main() {
  ...
  group('conditional effect removal', () {
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(_Inherited(value: 0, child: widget));
        expect(effectCalls, 0);
        expect(rebuilds.value, 1);
        ...
        await tester.pumpWidget(_Inherited(value: 1, child: widget));
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.pumpWidget(
          _Inherited(value: 0, child: widget),
        ...
        await tester.pumpWidget(
          _Inherited(value: 1, child: widget),
          duration: const Duration(milliseconds: 100),
        );
  ...
  group('keyed watchEffect() modifiers', () {
    testWidgets('watchEffect(immediate: true) calls the effect immediately', (
      tester,
    ) async {
      final notifier = ValueNotifier(0);
      var effectCalls = 0;
      final (widget, rebuilds) = _widget((context) {
        notifier.watchEffect(
          context,
          (value) {
            effectCalls++;
          },
      ...
      (tester) async {
        ...
        final (widget, rebuilds) = _widget((context) {
          notifier.watchEffect(
            ...
            key: 'effect',
            immediate: immediate.watch(context),
          );
          return const SizedBox.shrink();
        });
    ...
    testWidgets('watchEffect(once: true) calls the effect only once', (
      tester,
    ) async {
      final notifier = ValueNotifier(0);
      var effectCalls = 0;
  ...
  group('watchEffect() and watchOnly() interop', () {
    testWidgets('watchEffect() and watchOnly() can be used together', (
      tester,
    ) async {
      final notifier = ValueNotifier(0);
...
class _Inherited extends InheritedWidget {
  const _Inherited({required this.value, required super.child});
