...
for perm in permutations:
    ...
    for i, type in enumerate(perm):
        ...
        ext_on_types.append(f"context_ref.ReadOnlyRef<{type.replace('<T>', f'<T{i}>')}>")
    ...
    def watch(i, type):
        ...
        if ret == 'void':
            return f'${i + 1}.of(context).watch(context) as Null'
        return f'${i + 1}.of(context).watch(context)'

    extensions.append(textwrap.dedent(f'''
        /// More convenient API for watching multiple values at once.
        extension ContextWatchRecordRefExt{perm_index}{f'<{generics}>' if generics else ''} on ({on}) {{
...
combinations_dir_path = '../packages/context_plus/lib/src/watch_record_combinations'
...
with open('../packages/context_plus/lib/src/watch_record_combinations.dart', 'w+') as file:
