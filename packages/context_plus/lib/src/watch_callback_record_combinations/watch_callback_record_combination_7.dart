...
extension ContextWatchCallbackRecordRefExt7<T1, TListenable2 extends Listenable> on (context_ref.ReadOnlyRef<Stream<T1>>, context_ref.ReadOnlyRef<TListenable2>) {
  /// {@macro mass_watch_only_explanation}
  R watchOnly<R>(
    BuildContext context,
    R Function(AsyncSnapshot<T1>, TListenable2) selector,
  ) {
    final contextWatch = InheritedContextWatch.of(context);
    
    final observable1 = contextWatch.getOrCreateObservable<T1>(context, $1);
    if (observable1 == null) return selector(AsyncSnapshot<T1>.nothing(), $2.of(context));
  ...
  void watchEffect(
    BuildContext context,
    void Function(AsyncSnapshot<T1>, TListenable2) effect, {
    Object? key,
