...
for perm in permutations:
    ...
    for i, type in enumerate(perm):
        ...
        if type == 'Listenable':
            ext_on_types.append(f'TListenable{i + 1}')
        else:
            ext_on_types.append(type.replace('<T>', f'<T{i + 1}>'))
    ...
    def watch_effect_fetch_observable(i, type):
        ...
        if i == 0:
            lines.append(
                f'final obs{i + 1} = contextWatch.getOrCreateObservable{arg_type_generic}(context, ${i + 1});')
            lines.append(
    ...
    def unwatch_effect_calls():
        lines = []
        for i, _ in enumerate(perm):
            lines.append(f'contextWatch.unwatchEffect(context, ${i + 1}, key);')
        return '\n    '.join(lines)


    extensions.append(f'''
/// More convenient API for watching multiple values at once.
extension ContextWatchCallbackRecordExt{perm_index}{f'<{generics}>' if generics else ''} on ({on}) {{
...
dir_path = f'../packages/context_watch/lib/src/{dir_name}'
...
with open('../packages/context_watch/lib/src/watch_callback_record_combinations.dart', 'w+') as file:
