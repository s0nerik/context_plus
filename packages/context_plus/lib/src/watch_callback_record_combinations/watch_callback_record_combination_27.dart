...
extension ContextWatchCallbackRecordRefExt27<T1, T2, T3> on (context_ref.ReadOnlyRef<Future<T1>>, context_ref.ReadOnlyRef<Stream<T2>>, context_ref.ReadOnlyRef<Stream<T3>>) {
  /// {@macro mass_watch_only_explanation}
  R watchOnly<R>(
    BuildContext context,
    R Function(AsyncSnapshot<T1>, AsyncSnapshot<T2>, AsyncSnapshot<T3>) selector,
  ) {
    final contextWatch = InheritedContextWatch.of(context);
    
    final observable1 = contextWatch.getOrCreateObservable<T1>(context, $1);
    if (observable1 == null) return selector(AsyncSnapshot<T1>.nothing(), AsyncSnapshot<T2>.nothing(), AsyncSnapshot<T3>.nothing());
  ...
  void watchEffect(
    BuildContext context,
    void Function(AsyncSnapshot<T1>, AsyncSnapshot<T2>, AsyncSnapshot<T3>) effect, {
    Object? key,
